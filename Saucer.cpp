...
void Saucer::hit(const df::EventCollision *p_c) {
  ...
  if ((p_c -> getObject() -> getType() == "Saucer") &&
      (p_c -> getHitObject() -> getType() == "Saucer"))
    return;

  // If Bullet ...
  if ((p_c -> getObject() -> getType() == "Bullet") ||
	  (p_c->getHitObject()->getType() == "Bullet")) {
  ...
  if (((p_c -> getObject() -> getType()) == "Hero") || 
	  ((p_c->getHitObject()->getType()) == "Hero")) {
    df::WorldManager &world_manager = df::WorldManager::getInstance();
